...
Six steps to go from "just installed the app" to "looking at my report". Each step links to a more detailed page if you want the full story.
...
On first launch the app shows the **Disclaimer & Acknowledgement**. Scroll all the way to the bottom, then tap **Accept**.

More detail → [Disclaimer & Acknowledgement](disclaimer.md)

## 3. Sign in with an email code

1. Enter the email address Panthera has on file for you.
2. Tap **Email me a sign-in code**.
3. Open the email, copy the 6-digit code into the app, and tap **Verify and sign in**.
...
Right after the first sign-in, the app asks **"Stay signed in on this device?"** with two options:

- **Keep me signed in** (recommended) — the next launch unlocks with your device's PIN, fingerprint, or face. Quicker for daily use.
- **Sign in each time** — the session is wiped on exit; the next launch needs a fresh email code.

You can change this later in **Settings → On Exit → Keep me signed in**.

## 5. View your latest report
...
- Change theme, text size, or font → [Settings](settings.md)
- Turn the lock-on-resume gate on/off → [Settings → On Exit](settings.md#on-exit-keep-me-signed-in)
- Sign out / delete your account → [Settings](settings.md#sign-out)
